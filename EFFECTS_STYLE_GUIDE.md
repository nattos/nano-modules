...
**Custom-inspector widget choice: enums are TAB BARS, not dropdowns.** When you hand-roll an inspector, render `selectField`/enum params with **`<field-tab-bar>`** (every option visible, one click to pick) — *not* `<field-select>` (a dropdown: click → menu → hover → click). This matches what the generic inspector does for `selectField` (see line above), so custom and generic panels look the same. The only exception is a *genuinely large* set (**~10+** options, e.g. blend modes) where a strip would be unreadable — then a dropdown is fine. For medium sets pass `?wrap=${true}` so the segments flow onto multiple rows instead of squeezing. (This has bitten us repeatedly — the reflex to reach for a `<select>` is wrong here.) If the pick needs custom side-effects (e.g. a preset that writes sibling fields in one undo via `beginContinuousEditMulti().accept()`), replicate the `<field-tab-bar>` look with plain buttons rather than falling back to a `<select>` — see `web/src/editors/lens-inspector.ts` (the preset picker).
