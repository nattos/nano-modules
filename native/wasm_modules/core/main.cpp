...
void nano_module_main() {
    ...
    nano::registerEffect({
        ...
        "Three Planes Rig",
        "Show controller for source.mesh.three_planes. Reads four on/off signals (a beatsync Art-Net drum feed, a sequencer, four buttons) as an EV meter with peak hold: each signal names a floor, a hit throws the meter up to it and the meter falls back on its own time, leaving a held cap behind in the Highlight colour. Each hit also flams its floor — a short brightness blip that swings the colour to the other end. Publishes three emission rails, three colour rails, and orbit / elevation / spacing, all normalised so a plain wire lands on the right value. Four monophonic one-shot camera moves (Show, Sweep Up, Glance, Unfold) compose on top, in every mode. A second mode, Solid, drops the reactivity entirely: three floors lit in the three colours, nothing but the moves. The Sweep is a bipolar performance knob with a motion estimator behind it: where it sits dims the whole tower (wide centre deadzone, fading to black at either end with the floors stuttering one at a time on the way out), and how fast you move it throws travelling diagonal glints over in three_planes, which reads the knob itself off the sweep_out rail (they are objects thrown by the gesture, not a level). Pure data module. Reaching either extreme is a THROW rather than just a mute: passing through the middle charges by how hard you went through it (a short vigorous flick charges hardest, dawdling charges nothing), arriving at a mute spends it, and the release rings out on its own clock (three_planes flings the stack outward as expanding rings).",
        "mod",
    ...
    nano::registerEffect({
        ...
        "util.room_wrap",
        "Room Wrap",
        "One picture spread across the three walls of a room — a back screen and two side screens, from a single input, so a wipe or a field of light crosses all three continuously. The main output is the back wall: the input at square pixels, zoomed by Scale. Everything that zoom pushes off the left and right edges comes out of Left Out and Right Out, keystoned into a view of a wall running away from you — the outer end of each panel is the end nearest the viewer, so that is the end the picture opens out on. Perspective sets how deep the room is: at 1 the walls reach exactly as far as the picture, so the three screens tile it with nothing repeated or lost; at 0 they are a flat row with the overflow stretched evenly. The seams always join continuously. Shares its projection with util.triptych, which lays three panels out INTO a room the way this cuts one picture up for one. Side outputs cost nothing until wired.",
        "util",
