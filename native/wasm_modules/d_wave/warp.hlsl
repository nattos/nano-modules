...
//
// Reads the polar D-wave strength buffer (field.hlsl) and radially distorts the
// input texture by it: each pixel's centred coordinate is scaled in/out by a
// factor driven by the local field strength, so the expanding ripple rings pull
// the image inward in concentric arcs. Composites the distorted image over the
// untouched input by `render_alpha`.

Texture2D<float4>   inputTex : register(t0);
Texture2D<float4>   field    : register(t1);
SamplerState        sampIn   : register(s2);   // Linear + ClampToEdge
SamplerState        sampF    : register(s3);   // Linear + Repeat (angle wraps)
...
cbuffer Uniforms : register(b5) {
  ...
  float debug_field;   // overlay the raw field strength [0,1]
...
void main(uint3 gid : SV_DispatchThreadID) {
  ...
  // Polar lookup into the field: column = angle, row = radius.
  ...
  float LI  = field.SampleLevel(sampF, float2(frac(ang), row), 0).r;
  ...
  // Debug: overlay the raw field strength as a red ramp.
