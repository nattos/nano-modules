 ...
 * the per-bar values into uniforms and renders them.
 */
...
public:
  ...
  void setParams(const Params& p) {
    // Only the matrix STRUCTURE (routing) needs a rebuild; feedback and
    // decay_shaping are applied as a per-hop scalar gain, so they're free
  ...
  /// Inject `amount` of value into one bar (`bar` ∈ [0, N)).
  void impulse(int bar, float amount) {
    if (bar < 0 || bar >= N) return;
    v_[bar] += amount;
  }

  /// Inject `amount` into every bar (a uniform splash).
  void impulseAll(float amount) {
    for (int b = 0; b < N; b++) v_[b] += amount;
  }
  ...
  void step(float dt) {
    ...
    for (int s = 0; s < hops; s++) {
      const float (*M)[N] = M_[hop_idx_ % nc];
      ...
      // Pure redistribution (column sum 1) × the shaped feedback gain.
      float nv[N];
      for (int i = 0; i < N; i++) {
        float a = 0.0f;
        for (int j = 0; j < N; j++) a += M[i][j] * v_[j];
        nv[i] = a * fb;
      }
      for (int i = 0; i < N; i++) {
