...
void main(uint3 gid : SV_DispatchThreadID) {
  ...
  // not look alike.
  bool wired = (col == 0) ? (misc.z > 0.5) : (col == 2 ? (misc.w > 0.5) : true);
  if (!wired) {
    outputTex[gid.xy] = float4(0, 0, 0, 0);
    return;
  }
  ...
  uint sw, sh;
  if (col == 0)      leftTex.GetDimensions(sw, sh);
  else if (col == 2) rightTex.GetDimensions(sw, sh);
  else               midTex.GetDimensions(sw, sh);

  float2 uv;
  if (!panel_uv(col_uv, float2(float(sw), float(sh)),
                float2(col_w, vp.y), mode, uv)) {
    outputTex[gid.xy] = float4(0, 0, 0, 0);
    return;
  }

  float4 c;
  if (col == 0)      c = leftTex.SampleLevel(samp, uv, 0);
  else if (col == 2) c = rightTex.SampleLevel(samp, uv, 0);
