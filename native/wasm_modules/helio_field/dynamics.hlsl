...
//   aux = (A, heat, 0, 0)   — magnetic scalar potential + storm afterglow
//
...
cbuffer DynUniforms : register(b5) {
  ...
  float recon;      // storm reconnection diffusion of A (0 until storms)
...
void main(uint3 gid : SV_DispatchThreadID) {
  ...
  if (reset > 0.5) {
    ...
    auxNext[gid.xy] = float4(target_A(dir), 0.0, 0.0, 0.0);
    return;
  }
  ...
  float Ac = auxPrev.Load(int3(gid.xy, 0)).x;
  ...
  float Fpu = aux_at(dpu).x, Fmu = aux_at(dmu).x;
  float Fpv = aux_at(dpv).x, Fmv = aux_at(dmv).x;

  // --- Lorentz tension: F = −∇²A · ∇A (kinked lines pull straight).
  // Saturated so a forming current sheet can't spike the integrator. ---
  float3 gA = (t1 * (Fpu - Fmu) + t2 * (Fpv - Fmv)) / (2.0 * force_eps);
  float lapA = (Fpu + Fmu + Fpv + Fmv - 4.0 * Ac) / (force_eps * force_eps);
  float3 fmag = (-lapA) * gA;
  ...
  float avgN = (Apu + Amu + Apv + Amv) * 0.25;
  A = lerp(A, avgN, resist);
  A += (target_A(dir) - A) * (emerge * dt);
  A += lapA * (recon * heat * dt);
  heat *= exp(-2.0 * dt);

  dynNext[gid.xy] = float4(vel, 0.0);
  auxNext[gid.xy] = float4(A, heat, 0.0, 0.0);
}
