// source.shape_fold — skip-empty debug visualizer.
//
// Overwrites tex_out with a per-tile heatmap of one of the detector features
// (variance / edge / motion / combined), so the weights can be tuned by eye.
// Reads the per-tile stats the edge pass just wrote (same submit) and replicates
// the CPU reduce math per tile — no tex_out read, so no read/write hazard. Only
// dispatched when the debug mode is non-zero (a tuning aid, not the normal path).
//
// NOTE: the decision path in main.cpp reduces MOTION by the global mean (not the
// per-tile MAX shown here); this heatmap is purely to SEE where motion/edges sit.
