// probe3_log.h — sibling of probe_log.h / probe2_log.h with its own
// log dir (~/Library/Logs/NanoProbe3), os_log subsystem, and namespace.
...
inline os_log_t& oslog() {
  static os_log_t l = os_log_create("com.nano.NanoProbe3", "probe");
  return l;
}
...
inline void ensure_open() {
  ...
  if (!home) return;
  std::string dir = std::string(home) + "/Library/Logs/NanoProbe3";
  ...
  file() = fopen(buf, "a");
  if (file()) {
    fprintf(file(), "# NanoProbe3 log opened pid=%d unixms=%llu path=%s\n",
...
#define PROBE_LOG(event, fmt, ...) \
  ::nano_probe3_log::log_line((event), (fmt), ##__VA_ARGS__)

#define PROBE_REDACT(...) ::nano_probe3_log::redact(__VA_ARGS__)

#define PROBE_CTX_FRAME(n) (::nano_probe3_log::ctx().frame = (n))
#define PROBE_CTX_PHASE(p) (::nano_probe3_log::ctx().phase = (p))
