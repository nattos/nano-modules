 ...
 * (instance key), surfaced in infoJson for UI labels. Bounded history: the
 * oldest event is dropped once the log is full (consumers drain every tick).
 */
 ...
 * {"on":bool,"velocity":num,"writer":tag,"seq":num}}}` — into `out` (capacity
