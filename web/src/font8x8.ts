...
// 96 glyphs covering ASCII 32-127. Each glyph: 8 bytes, bit 0 = rightmost pixel (standard LSB).
export const FONT_GLYPH_W = 8;
export const FONT_GLYPH_H = 8;
export const FONT_ATLAS_COLS = 16;
export const FONT_ATLAS_ROWS = 6;
export const FONT_ATLAS_W = 128;
export const FONT_ATLAS_H = 48;

// prettier-ignore
export const kFont8x8: number[][] = [
  ...
  [0x06,0x08,0x1C,0x08,0x08,0x08,0x08,0x00], // 102 'f'
  ...
  [0x00,0x00,0x1A,0x15,0x15,0x15,0x11,0x00], // 109 'm'
  ...
  [0x00,0x00,0x11,0x15,0x15,0x15,0x0A,0x00], // 119 'w'
  ...
  [0x06,0x08,0x08,0x10,0x08,0x08,0x06,0x00], // 123 '{'
  [0x04,0x04,0x04,0x04,0x04,0x04,0x04,0x00], // 124 '|'
  [0x0C,0x02,0x02,0x01,0x02,0x02,0x0C,0x00], // 125 '}'
  [0x00,0x08,0x15,0x02,0x00,0x00,0x00,0x00], // 126 '~'
  [0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00], // 127 DEL
];

export function buildFontAtlas(): Uint8Array {
  const pixels = new Uint8Array(FONT_ATLAS_W * FONT_ATLAS_H);
  for (let i = 0; i < 96; i++) {
    const col = i % FONT_ATLAS_COLS;
    const row = Math.floor(i / FONT_ATLAS_COLS);
    for (let py = 0; py < FONT_GLYPH_H; py++) {
      const bits = kFont8x8[i][py];
      for (let px = 0; px < FONT_GLYPH_W; px++) {
        if (bits & (1 << px)) {
          const ax = col * FONT_GLYPH_W + (FONT_GLYPH_W - 1 - px);
          const ay = row * FONT_GLYPH_H + py;
          pixels[ay * FONT_ATLAS_W + ax] = 255;
        }
      }
    }
  }
