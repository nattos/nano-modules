...
export class ArrangementStore {
  ...
  toggleSolo(trackId: string, additive = false) {
    this.mutate('toggle solo', (d) => {
      const t = d.tracks.find((x) => x.id === trackId);
      if (!t) return;
      // A return/rail has no picture — soloing it renders black, so it never solos
      // ON. Hitting solo on a return instead acts as "clear solo": it turns OFF every
      // track's solo (returning the mix to normal).
      if (t.kind === 'rail') { for (const x of d.tracks) x.soloed = false; return; }
